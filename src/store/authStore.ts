...
interface AuthState {
  ...
  updateUserImage: (imageFile: File) => Promise<void>; // ¡NUEVA ACCIÓN: Declaración en la interfaz!
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  register: async (userData: RegisterRequestFrontend) => {
    ...
    try {
      ...
      if (response?.token) {
        // Asumo que el registro devuelve un token y un usuario directamente para login
        localStorage.setItem("jwt_token", response.token);
        set({
          isAuthenticated: true,
          token: response.token,
  ...
  checkAuth: async () => {
    ...
    if (token) {
      ...
      console.log(
        "Token found in localStorage, setting isAuthenticated to true."
      );
      ...
      console.log(
        "No token found or token expired/invalid, setting isAuthenticated to false."
      );
    }
  },
  ...
  updateUserImage: async (imageFile: File) => {
    ...
    try {
      const updatedUserWithImage = await authService.uploadProfileImage(
        token,
        imageFile
      ); // updatedUserWithImage es ahora UserDTO
  ...
  updateUserCredentials: async (credentials: UpdateCredentialsRequest) => {
    ...
    if (!token) {
      set({
        loadingUser: false,
        errorUser:
          "No hay token de autenticación para actualizar credenciales.",
      });
      throw new Error("No hay token de autenticación.");
    }
    try {
  ...
  // *** NUEVA ACCIÓN: Implementación de deactivateAccount ***
  deactivateAccount: async () => {
    set({ loadingUser: true, errorUser: null }); // Usamos loadingUser para indicar actividad en el perfil
    const token = get().token;
    if (!token) {
      set({
        loadingUser: false,
      ...
      get().logout(); // Cerrar sesión si no hay token
      return;
    }
    ...
    try {
      ...
      get().logout(); // Cerrar sesión inmediatamente después de desactivar
    } catch (error: any) {
      ...
      throw error; // Re-lanza el error si es necesario para el componente
    }
  },
}));
