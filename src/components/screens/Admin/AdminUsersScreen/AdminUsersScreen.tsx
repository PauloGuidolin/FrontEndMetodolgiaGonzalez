  ...
  const handleToggleActive = async (userId: number, currentStatus: boolean) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Error",
          `Error al cambiar el estado del usuario: ${error || "Desconocido"}`,
          "error"
        );
      }
    }
  ...
  const handleChangeRole = async (userId: number, newRole: Rol) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Error",
          `Error al cambiar el rol del usuario: ${error || "Desconocido"}`,
          "error"
        );
      }
    }
  ...
  const handleHardDelete = async (userId: number) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire(
          "Error",
          `Error al eliminar físicamente el usuario: ${error || "Desconocido"}`,
          "error"
        );
      }
    }
