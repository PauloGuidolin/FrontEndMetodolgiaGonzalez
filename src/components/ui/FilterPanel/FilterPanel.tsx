...
// Importamos Sexo si lo usas como tipo en ProductFilters y necesitas manejarlo
...
export const FilterPanel: React.FC<FilterPanelProps> = ({
    ...
    const [selectedFilters, setSelectedFilters] = useState<ProductFilters>(() => {
        ...
        const combinedFilters: ProductFilters = {
            ...
            categorias: Array.isArray(initialFilters?.categorias) ? initialFilters.categorias : [], // <-- Usar 'categorias'
            colores: Array.isArray(initialFilters?.colores) ? initialFilters.colores : [],    // <-- Usar 'colores'
            talles: Array.isArray(initialFilters?.talles) ? initialFilters.talles : [],      // <-- Usar 'talles'
    ...
    const handleCheckboxChange = (filterName: 'categorias' | 'colores' | 'talles', value: string, isChecked: boolean) => {
        setSelectedFilters(prevFilters => {
    ...
    return (
        <div className={`${styles.filterPanelOverlay} ${isOpen ? styles.isOpen : ''}`} onClick={onClose}>
            ...
            <div className={`${styles.filterPanel} ${isOpen ? styles.isOpen : ''}`} onClick={(e) => e.stopPropagation()}>
                <div className={styles.filterPanelHeader}>
                    ...
                    <button className={styles.closeFilterButton} onClick={onClose}>X</button>
                </div>
                <div className={styles.filterPanelContent}>
                    ...
                    <div className={styles.filterSection}>
                        <h3>ORDENAR POR</h3>
                        <select className={styles.filterSelect} value={selectedFilters.orderBy ?? ''} onChange={(e) => handleFilterChange('orderBy', e.target.value === '' ? null : e.target.value)}>
                            <option value="">Por defecto</option>
                            <option value="precioVenta">Precio</option> {/* Asegúrate que 'precioVenta' o 'precioOriginal' sea el campo por el que ordenas en backend */}
                        ...
                        <select className={styles.filterSelect} value={selectedFilters.orderDirection ?? ''} onChange={(e) => handleFilterChange('orderDirection', e.target.value === '' ? null : e.target.value as 'asc' | 'desc')}>
                            <option value="">Por defecto</option>
                    ...
                    <div className={styles.filterSection}>
                        <h3>SEXO</h3>
                        <select className={styles.filterSelect} value={selectedFilters.sexo ?? ''} onChange={(e) => handleFilterChange('sexo', e.target.value === '' ? null : e.target.value)}>
                    ...
                    <div className={styles.filterSection}>
                        ...
                        {Array.isArray(availableCategories) && availableCategories.length === 0 && <p>No hay categorías disponibles.</p>}
                         {Array.isArray(availableCategories) && availableCategories.map((categoryName, index) => {

                             if (!categoryName || typeof categoryName !== 'string') {
                                 console.warn("Skipping rendering for invalid category item:", categoryName);
                             ...
                             return (
                                 <div key={key} className={styles.checkboxItem}>
                                     <input
                                         ...
                                         onChange={(e) => handleCheckboxChange('categorias', categoryName, e.target.checked)} // <-- Usar 'categorias'
                                     />
                    ...
                    <div className={styles.filterSection}>
                         ...
                         {Array.isArray(availableColors) && availableColors.length === 0 && <p>No hay colores disponibles.</p>}
                         {Array.isArray(availableColors) && availableColors.map((colorName, index) => {

                             if (!colorName || typeof colorName !== 'string') {
                                 console.warn("Skipping rendering for invalid color item:", colorName);
                                 return null;
                             }
                             ...
                             return (
                                 <div key={key} className={styles.checkboxItem}>
                                     <input type="checkbox" id={`color-${key}`} value={colorName}
                                          ...
                                          onChange={(e) => handleCheckboxChange('colores', colorName, e.target.checked)} // <-- Usar 'colores'
                                      />
                    ...
                    <div className={styles.filterSection}>
                         ...
                         {Array.isArray(availableSizes) && availableSizes.length === 0 && <p>No hay tallas disponibles.</p>}
                         {Array.isArray(availableSizes) && availableSizes.map((sizeName, index) => {

                             if (!sizeName || typeof sizeName !== 'string') {
                                 console.warn("Skipping rendering for invalid size item:", sizeName);
                                 return null;
                             }
                             ...
                             return (
                                 <div key={key} className={styles.checkboxItem}>
                                     <input type="checkbox" id={`size-${key}`} value={sizeName}
                                          ...
                                          onChange={(e) => handleCheckboxChange('talles', sizeName, e.target.checked)} // <-- Usar 'talles'
                                      />
